...
function initArea(){												//生成选座区
    ...
    for(var i = 0; i < temArr.length; i++){
    	...
    	var innerHTML = '<label for="input-total_tickets_'+temArr[i]+'" class="col-sm-2 control-label">'+temArr[i]+'区票数</label> <div class="col-sm-2"> <input type="number" value = "0" name="total_tickets_'+temArr[i]+'" class="form-control" id="input-total_tickets_'+temArr[i]+'" min="0" placeholder = "'+temArr[i]+'区发票总数" readonly="readonly"> </div> <div class = "col-sm-8">  <input class="form-control" type = "text" value = "" name = "seats_for_choose_'+temArr[i]+'" id="seats_for_choose_'+temArr[i]+'" min="0" readonly="readonly"> </div>';
	...
	$(".checkArea").change(function(e){
		...
		if($("#"+targetId).is(":checked") == true){			//全选
			...
			document.getElementById("seats_for_choose_"+area).value = seats_to_publish.filter(function(item){return startsWith(item, area)});
		}
		else{												//全不选
...
function is_XTHXT_seat(seat_id){
	...
	//非新清华学堂的座位
	var arr_not_XTHXT_seat = [0, 1, 30, 28, 29, 59];
